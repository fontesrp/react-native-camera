...
import type { Rationale, ViewProps } from 'react-native'
...
type PropsType = ViewProps & {
  ...
  pendingAuthorizationView: ReactNode
  notAuthorizedView: ReactNode
}
...
const CameraStatus: { [K in Status]: Status } = {
  READY: 'READY',
...
const RecordAudioPermissionStatusEnum: {
  [K in RecordAudioPermissionStatus]: RecordAudioPermissionStatus
} = {
...
const CameraManager: CameraManagerInterface = NativeModules.RNCameraManager ||
...
const RationalePropType = PropTypes.shape({
...
const RNCamera = requireNativeComponent<RNCameraNativeProps>('RCTCamera')
...
export default class Camera extends React.Component<PropsType, StateType> {
  ...
  static propTypes = {
    ...
    androidCameraPermissionOptions: RationalePropType,
    androidRecordAudioPermissionOptions: RationalePropType,
  ...
  async arePermissionsGranted() {
    ...
    const { hasCameraPermissions, hasRecordAudioPermissions } = await requestPermissions(
      ...
      CameraManager,
      cameraPermissions as Rationale,
      audioPermissions as Rationale
    )
  ...
  async refreshAuthorizationStatus() {
    const { hasCameraPermissions, recordAudioPermissionStatus } = await this.arePermissionsGranted()
  ...
  _convertNativeProps({ children, ...props }: PropsType): { [key: string]: any } {
    ...
    return { ...defaultNativeProps, ...newProps }
  }
...
export { CameraConstants as Constants }
